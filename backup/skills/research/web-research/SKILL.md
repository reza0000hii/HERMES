...
30. **`article:published_time` works on Deadline but NOT on Variety/THR** — Deadline article pages are server-rendered and reliably expose `<meta property="article:published_time" content="...">` with ISO 8601 timestamps. However, Variety and THR article pages are fully JS-rendered — this meta tag either doesn't appear or isn't populated in the raw HTML. For Variety/THR date verification, rely exclusively on RSS feeds `<pubDate>`. When the user asks "what was published TODAY", use RSS first, then `article:published_time` only for Deadline.
